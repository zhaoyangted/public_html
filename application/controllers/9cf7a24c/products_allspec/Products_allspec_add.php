<?php
class Products_allspec_add extends CI_Controller {
	...
	public function index(){
        ...
        // $this->tableful->TableTreat(0);
				$this->db->query('SET SESSION group_concat_max_len = 1000000');	// 2020/03/31增加查詢長度

        $Pdata=$this->mymodel->Writesql('SELECT GROUP_CONCAT(PID) as PID FROM products_allspec where PID !=""','1');
	...
	public function add(){
        ...
        if(!empty($Cck)){
            ...
            return '';
				...
				/*特殊檢查位置-總碼名稱不能重複*/
				$chktitle=$this->mymodel->OneSearchSql($this->DBname,'d_title',array('d_title'=>$_POST['d_title']));
				if (!empty($chktitle)) {
