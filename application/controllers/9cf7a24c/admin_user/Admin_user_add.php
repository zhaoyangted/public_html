<?php
class Admin_user_add extends CI_Controller {
	...
	public function index(){
        ...
        // 特殊欄位處理
        $this->tableful->TableTreat(0,'and d_id!=1');
	...
	public function add(){
        ...
        /*特殊檢查位置*/
        $Chk=$this->mymodel->OneSearchSql($dbname,'d_id',array('d_account'=>$post['d_account']));
        if(!empty($Chk)){
            $this->useful->AlertPage('','已有相同帳號，請重新輸入');
            return '';
        }
        $this->load->library('encryption');//加密
        $dbdata['d_password']=$this->encryption->encrypt($dbdata['d_password']);
        /*特殊檢查位置*/
