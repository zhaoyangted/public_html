<?php
class Products_gift extends CI_Controller {
	...
	public function index(){
		...
		$dbdata=$this->mymodel->SelectPage($this->DBname,$this->tableful->SqlList.',PID',$this->tableful->WhereSql,'d_sort');

		foreach ($dbdata['dbdata'] as $key => $value) {
			if($value['d_type']==2){
				$Pdata=$this->mymodel->OneSearchSql('products','d_title',array('d_id'=>$value['PID']));
