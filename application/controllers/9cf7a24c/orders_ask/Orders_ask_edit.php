<?php
class Orders_ask_edit extends CI_Controller {
    ...
    public function Sendmail($dbname,$d_id){
        $dbdata1=$this->mymodel->OneSearchSql('orders_ask','d_email,d_answer,d_content',array('d_id'=>$d_id));
        $CTitle=$this->webmodel->BaseConfig('6');
        $Mail=$dbdata1['d_email'];
        $Subject=$CTitle['d_title'].'-訂單詢問回覆信';
        $Message ='
            您好，您的問題是， <br>
                '.stripslashes($dbdata1['d_content']).'<br>
            -------------------------------- <br>
            本公司回覆如下: <br>
                '.stripslashes($dbdata1['d_answer']).'   <br>
        ';
