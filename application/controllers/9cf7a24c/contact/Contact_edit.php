<?php
class Contact_edit extends CI_Controller {
	...
	public function index($d_id) {
		if (!empty($d_id)) {
			...
			$this->tableful->TableTreat(5);


			$data['d_id'] = $d_id;
			$dbdata = $this->mymodel->OneSearchSql($this->DBname, $this->tableful->SqlList.',d_zipcode,d_county,d_district', array('d_id' => $d_id));
			$dbdata['d_address'] = $dbdata['d_zipcode'].$dbdata['d_county'].$dbdata['d_district'].$dbdata['d_address'];
			$data['dbdata'] = $dbdata;
	...
	public function edit() {
		...
		// 寄回覆信
    if($dbdata['d_status']==2)
        $this->Sendmail($dbname,$d_id);
	...
	// 寄回覆信
    public function Sendmail($dbname,$d_id){
        $dbdata1=$this->mymodel->OneSearchSql('contact','d_mail,d_reply,d_content',array('d_id'=>$d_id));
        $CTitle=$this->webmodel->BaseConfig('6');
        $Mail=$dbdata1['d_mail'];
        $Subject=$CTitle['d_title'].'-聯絡我們回覆信';
        $Message ='
            您好，您的問題是， <br>
                '.stripslashes($dbdata1['d_content']).'<br>
            -------------------------------- <br>
            本公司回覆如下: <br>
                '.stripslashes($dbdata1['d_reply']).'   <br>
        ';
