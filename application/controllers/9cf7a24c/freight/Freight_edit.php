<?php
class Freight_edit extends CI_Controller {
	...
	public function index($d_id){
        if(!empty($d_id)){
            ...
            if($d_id!=1 && $d_id!=6){
                unset($this->tableful->Menu[2]);
            }
						// 特殊運費拔除 離島及運費設定
						if($d_id==7){
							unset($this->tableful->Menu[3]);
							unset($this->tableful->Menu[4]);
						}
    ...
    public function deletefile(){
        if($_POST['deltype']=='Y'){
            $dbname=$_POST['dbname'];
            if($_POST['d_id']==1||$_POST['d_id']==6||$_POST['d_id']==7){
                $this->useful->AlertPage($this->AdminName.'/'.$dbname.'/'.$dbname,'此資料不可刪除');
                exit();
            }
