<?php
class Products_type_ss_add extends CI_Controller {
	...
	public function add(){
        ...
        if(!empty($msg)){
            $this->useful->AlertPage($this->AdminName.'/'.$dbname.'/'.$dbname.'_ss/index/'.$dbdata['TID'].'/'.$dbdata['TTID'],'新增完成');
        }else{
            $this->useful->AlertPage($this->AdminName.'/'.$dbname.'/'.$dbname.'_ss/index/'.$dbdata['TID'].'/'.$dbdata['TTID'],'新增失敗');
        }
    }
}