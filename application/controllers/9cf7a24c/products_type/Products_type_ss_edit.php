<?php
class Products_type_ss_edit extends CI_Controller {
	...
	public function index($d_id){
        if(!empty($d_id)){
            ...
            // 撈取上層標題
            $Tdata=$this->mymodel->WriteSql('select REPLACE(group_concat(d_title),",",">") as d_title from products_type where d_id in ('.$dbdata['TID'].','.$dbdata['TTID'].')','1');
            $data['Uptitle']=$Tdata['d_title'];

            // $this->load->view(''.$this->AdminName.'/autopage/s_info',$data);
            $this->load->view(''.$this->AdminName.'/'.$this->DBname.'/ss_info',$data);
        }else
	...
	public function edit(){
        ...
        $this->useful->AlertPage($this->AdminName.'/'.$dbname.'/'.$dbname.'_ss/index/'.$dbdata['TID'].'/'.$dbdata['TTID'],'修改成功');
    }
}