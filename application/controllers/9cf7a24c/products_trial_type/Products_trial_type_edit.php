<?php
class Products_trial_type_edit extends CI_Controller {
    ...
    public function deletefile(){
        if($_POST['deltype']=='Y'){
            $dbname=$_POST['dbname'];
						// 特殊處理
						$d_id=$_POST['d_id'];
						$Tdata=$this->mymodel->OneSearchSql('products_trial','d_id',array('TID'=>$d_id));
						if (!empty($Tdata)) {
							$this->useful->AlertPage('','該專案底下尚有試用品，無法直接刪除專案！');
							exit();
						}
