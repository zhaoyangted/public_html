...
class Cart extends RestController
{
    ...
    public function cart_post($cart="")
    {
        //$post = array('d_account'=>$this->post('d_account'),'d_password'=>$this->post('d_password'));
        $cart = $this->post('cart');
        //$cart = $post['cart'];
        //$Cart = (!empty($_COOKIE['BeautyCart']) ? $_COOKIE['BeautyCart'] : '');
        $this->load->library('Mylib/Cartful', 'cartful');
        $this->cartful->GetCart($cart);
        $data = array();
        // 數量
        $data['CartNum'] = $this->cartful->CartNum;
        // 產品
        $data['Cart']= array_merge($this->cartful->Order['Cart'], $this->cartful->Order['TrialData']);
        $data['CartTotal'] = $this->cartful->Order['Total'];
        $data['BonusTotal'] = $this->cartful->Order['BonusTotal'];
        // 一般運費
        $data['OneFreight'] = $this->cartful->Order['OneFreight'];
        $data['ChkTri'] = $this->cartful->Order['ChkTri'];
        $data['Freight'] = $this->cartful->Order['Freight'];
        $data['AddData'] = $this->cartful->Order['AddData'];
        $data['BigFreight'] = $this->cartful->Order['BigFreight'];
        $data['Outisland'] = $this->cartful->Order['Outisland'];
        $data['Special'] = $this->cartful->Order['Special'];
        $data['TrialData'] = $this->cartful->Order['TrialData'];
        $data['Subbonus'] = $this->cartful->Order['Subbonus'];
        //print_r($data);
        if (!empty($data)){
    ...
    // Step3 訂單資訊填寫
    public function cart_info_post($cart="")
    {
        // 購物車資訊
        //$cart = $this->chkCart();
        $cart = $this->post('cart');
    ...
    public function addorder_post()
    {
        ...
        // 離島另收
        $Freight = (int)$CartProduct['Freight'] + (int)$CartProduct['BigFreight'] + (int)($OrderData['d_logistics'] == 2 ? (int)$CartProduct['Outisland'] : 0);
        // 訂單總計 需+離島運費
        $CartProduct['AllTotal'] = (int)$CartProduct['Total'] + (int)$Freight - (int)$Subbonus;
        // 檢查紅利
        ...
        if ($CartProduct['Special'] == true) {
            ...
            $config['AuthResURL'] = base_url('paymentres');
            $this->load->library('Cash_flow', $config);
            // 傳送至金流
            $data = $this->cash_flow->creditCard_getForm_data();
            ...
            $accdata['total']=$CartProduct['AllTotal'];
            $this->response(accdata,200/* ,['msg'=>'請通過連結付款。'] */);
    ...
    private function chkBonus($Bonus, $AllTotal, $freight)
    {
        ...
        $Total = ((int)$AllTotal + (int)$freight) - (int)$Bonus;
        if ($Total < 0) {
