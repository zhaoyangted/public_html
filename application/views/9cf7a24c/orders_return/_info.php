...
<div class="indexView">
    <div class="indexView_block">
              ...
              <div style="overflow-x:scroll;">
                    ...
                    <?php foreach($oddata as $akey=> $avalue):?>
                        <tr>
                          ...
                          <td ><?php echo $Ostatus[$avalue['d_status']] ?></td>
                          <td ><?php echo !empty($Oship_freight[$avalue['d_pfreight_lv']])?$Oship_freight[$avalue['d_pfreight_lv']]:'無資料'; ?></td>
                          <td ><?php echo $avalue['d_pfreight'];?></td>
                          <td ><?php echo $avalue['d_ship_date'];?></td>
                          <td ><?php echo $avalue['d_arrival_date'];?></td>
                          <td ><?php echo $avalue['d_shipnumber'];?></td>
                          <td ><?php echo !empty($Oship_company[$avalue['SHID']])?$Oship_company[$avalue['SHID']]:'無資料'; ?></td>
                        </tr>
                        <?endforeach;?>
                  </table>
              </div>
              ...
              ?>
                  ...
                  <?if($fvalue['d_type']==1):?>
                      ...
                      <li>
                          ...
                          <input type="text" class="form-control" name="<?=$fvalue['d_fname']?>" id="<?=$fvalue['d_fname']?>" value="<?php echo !empty($dbdata[$fvalue['d_fname']])?$dbdata[$fvalue['d_fname']]:'';?>" <? if(!empty($fvalue['InputType'])):?>onkeyup="return <? echo $fvalue['InputType']?>($(this),value)"<? endif;?>>
                  ...
                  <?if($fvalue['d_type']==2):?>
                      ...
                      <li>
                          ...
                          <?foreach ($fvalue['Config'] as $Ckey=> $Cvalue):?>
                            <input value="<?=$Ckey?>" name="<?=$fvalue['d_fname']?>" id="<?=$fvalue['d_fname'].$Ckey?>" <? echo !empty($dbdata[$fvalue['d_fname']])?($dbdata[$fvalue['d_fname']]==$Ckey)?'checked':'':($Ckey==1 or $Ckey=='Y')?'checked':'';?> type="radio"><label for="<?=$fvalue['d_fname'].$Ckey?>" style="margin-right: 5px;"><?=$Cvalue?></label>
                          <? endforeach;?>
                          <? if(!empty($fvalue['d_content'])):?>
                              <span class="form-remark"><? echo $fvalue['d_content'];?></span>
                          <?endif;?>
                  ...
                  <?if($fvalue['d_type']==3):?>
                      ...
                      <li>
                          <div class="form_wrap_title"><?=$Star.$fvalue['d_title']?></div>
                          <?foreach ($fvalue['Config'] as $Ckey=> $Cvalue):?>
                            <input value="<?=$Ckey?>" name="<?=$fvalue['d_fname']?>[]" id="<?=$fvalue['d_fname'].$Ckey?>" <? echo !empty($dbdata[$fvalue['d_fname']])?(in_array($Ckey,explode('@#',$dbdata[$fvalue['d_fname']])))?'checked':'':'';?> type="checkbox"><label for="<?=$fvalue['d_fname'].$Ckey?>" style="margin-right: 5px;"><?=$Cvalue?></label>
                          <? endforeach;?>
                          <? if(!empty($fvalue['d_content'])):?>
                              <span class="form-remark"><? echo $fvalue['d_content'];?></span>
                          <?endif;?>
                  ...
                  <?if($fvalue['d_type']==7):?>
                      ...
                      <li>
                          <div class="form_wrap_title"><?=$Star.$fvalue['d_title']?></div>
                          <?php if ($fvalue['d_fname']=='OID'): ?>
                          <?php echo (!empty($fvalue['Config'])?(!empty($fvalue['Config'][$dbdata[$fvalue['d_fname']]])?$fvalue['Config'][$dbdata[$fvalue['d_fname']]]:'無資料'):(isset($dbdata[$fvalue['d_fname']])?'<a href="'. CCODE::DemoPrefix.'/'.(!empty($this->autoful->FileName)?$this->autoful->FileName:'admin_sys').'/orders/orders_edit/index/'.$dbdata['d_id'].'">'.$dbdata[$fvalue['d_fname']].'</a>':''));?>
                          <?php else: ?>
                  ...
                  <?if($fvalue['d_type']==18):?>
                      ...
                      <li>
                          <div class="form_wrap_title"><?=$Star.$fvalue['d_title']?></div><br>
                          <input type="number" class="form-control" name="<?=$fvalue['d_fname']?>" id="<?=$fvalue['d_fname']?>" min="0" value="<? echo !empty($dbdata[$fvalue['d_fname']])?$dbdata[$fvalue['d_fname']]:0;?>" style="width: 100px;">
                      </li>
                  <?endif;?>
                  <?if($fvalue['d_type']==19):?>
                      <!--顏色欄位-->
        ...
        <div class="search-btn">
            ...
            <input type="button" value="回上頁" class="other" onclick="javascript:window.location.href='<?echo CCODE::DemoPrefix.'/'.((!empty($this->autoful->FileName)?$this->autoful->FileName:'admin_sys').'/'.$this->DBname.'_return/'.$this->DBname.'_return');?>'">
        </div>
  </div>
</div>
