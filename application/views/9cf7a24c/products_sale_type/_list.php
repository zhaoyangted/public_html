  ...
  <div class="indexView_block">
    <div class="indexView_title">
      ...
      <?endif;?>
        <div class="indexView_block_search">
            <div class="search-btn">
              <a class="other" href="javascript:void(0)" id="DownSale">匯出報表</a>
            </div>
        </div>
        <div class="toolbar">
          <?if($this->autoful->EditView==2):?>
            ...
            <?endif;?>
            <a href="avascript:void(0)" style="margin-left: 5px" class="toolbar-btn copy" onclick="if(confirm('確定複製資料?') == false){return false;}else{DataCopy()}">複製</a>
        ...
        <tbody>
          ...
          <?php elseif($AutoValue['d_type']==10): ?>
            ...
            <td class="text-center">
              <a href="<? echo CCODE::DemoPrefix.'/'.((!empty($this->autoful->FileName)?$this->autoful->FileName:'admin_sys').'/'.str_replace("_type","",$this->DBname).'/'.str_replace("_type","",$this->DBname).'/index/'.$dbval['d_id']);?>"><img class="ico-operating" src="<? echo CCODE::DemoPrefix.'/'.('images/backend/ico_p_edit.png')?>"></a>
            </td>
            <?if($this->tableful->MenuidDb['d_edit']=='Y'):?>
              <td class="text-center">
...
<script>
  function DataCopy() {
    ...
    $.ajax({
        url:$('#FileName').attr('fval')+'/AConfig/AConfig/DataCopy',
        type:'POST',
        data: 'DB='+DB+'&id='+str,
        dataType: 'text',
  ...
  $('#DownSale').click(function(){
    $('#SearchForm').attr('action','products_sale_type/DownSale').submit();
