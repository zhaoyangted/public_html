        ...
        <div class="toolbar">
          ...
          <?if($this->autoful->EditView==2):?>
            <?if($this->tableful->MenuidDb['d_add']=='Y'):?>
              <a href="<? echo CCODE::DemoPrefix.'/'.((!empty($this->autoful->FileName)?$this->autoful->FileName:'admin_sys').'/'.$this->DBname.'/'.$this->DBname.'_ss_add/index/'.$TID.'/'.$TTID.'');?>" class="toolbar-btn add">+新增</a>
            <?endif;?>
          <?endif;?>
            <a href="<? echo CCODE::DemoPrefix.'/'.((!empty($this->autoful->FileName)?$this->autoful->FileName:'admin_sys').'/'.$this->DBname.'/'.$this->DBname.'_s/index/'.$TID);?>" class="toolbar-btn copy" style="float: right;">返回</a>
        </div>
          ...
          <?php elseif($AutoValue['d_type']==10): ?>
            ...
            <?if($this->tableful->MenuidDb['d_edit']=='Y'):?>
              <td class="text-center">
                <a href="<? echo CCODE::DemoPrefix.'/'.((!empty($this->autoful->FileName)?$this->autoful->FileName:'admin_sys').'/'.$this->DBname.'/'.$this->DBname.'_ss_edit/index/'.$dbval['d_id']);?>"><img class="ico-operating" src="<? echo CCODE::DemoPrefix.'/'.('images/backend/ico_p_edit.png')?>"></a>
              </td>
            <?endif;?>
            <?if($this->autoful->EditView==2):?>
              <?if($this->tableful->MenuidDb['d_del']=='Y'):?>
