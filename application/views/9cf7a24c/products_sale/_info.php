  ...
  <div class="indexView">
      <div class="indexView_block">
              ...
              <div class="form-content">
                ...
                ?>
                    ...
                    <?if($fvalue['d_type']==7):?>
                        ...
                        <li>
                            <div class="form_wrap_title"><?=$Star.$fvalue['d_title']?></div>
                            <?php echo (!empty($fvalue['Config'])?$fvalue['Config'][$dbdata[$fvalue['d_fname']]]:($dbdata[$fvalue['d_fname']]!=""?$dbdata[$fvalue['d_fname']]:''));?>
                        </li>
                    <?endif;?>
                    <?if($fvalue['d_type']==8):?>
                        <!--Input Imgfile -->
                    ...
                    <?if($fvalue['d_type']==17):?>
                        <!--搜尋下拉-->
                          <li>
                              <div class="form_wrap_title"><?=$Star.$fvalue['d_title']?></div>
                              <select id="<?=$fvalue['d_fname']?>" <?= ($ChkEnd)?'disabled':'name="'.$fvalue['d_fname'].'[]"'; ?> multiple class="form-select" <?echo $Disabled;?>>
                                  <? foreach ($fvalue['Config'] as $Ckey=> $Cvalue):?>
                    ...
                    <?if($fvalue['d_type']==18):?>
                        ...
                        <li>
                            <div class="form_wrap_title"><?=$Star.$fvalue['d_title']?></div><br>
                            <input type="number" class="form-control" name="<?=$fvalue['d_fname']?>" id="<?=$fvalue['d_fname']?>" <?=($fvalue['d_fname']=='d_price')?'step="0.01"':''; ?> min="0" value="<? echo !empty($dbdata[$fvalue['d_fname']])?$dbdata[$fvalue['d_fname']]:0;?>" style="width: 100px;" <?echo $Disabled;?>>
                            <? if(!empty($fvalue['d_content'])):?>
                                <span class="form-remark"><? echo $fvalue['d_content'];?></span>
                            <?endif;?>
                            <? if ($fvalue['d_fname']=='d_price'): ?>
                              <br>
                              <p>試算：商品金額<input style="display:inline-block;width: 100px;" type="number" min="0" onchange="percent()" value="" class="form-control" id="order_all" <?echo $Disabled;?>/>，最後特價為<span id="percent"></span></p>
                            <?php endif; ?>
                        </li>
                    <?endif;?>
                    <?if($fvalue['d_type']==19):?>
          ...
          <div class="search-btn">
              ...
              <input type="button" value="回上頁" class="other" onclick="javascript:window.location.href='<?echo CCODE::DemoPrefix.'/'.((!empty($this->autoful->FileName)?$this->autoful->FileName:'admin_sys').'/'.$this->DBname.'/'.$this->DBname.'/index/'.$dbdata['TID'].'');?>'">
...
$('input[name="d_price"],#order_all').change(function() {
  var rebate = Math.floor($('input[name="d_price"]').val()/100*$('#order_all').val());
  $('#percent').html($('#order_all').val()+'*'+$('input[name="d_price"]').val()+'% = '+rebate+' 元');
});
