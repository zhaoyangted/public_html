...
<script>
$('#DownPlow').click(function(){
  $('#SearchForm').attr('action','plow/Export').submit();
});
$('#Excel_Import').click(function(){
  if(document.getElementById("excel_upload").files.length == 0){
    alert("請選擇檔案！");
  }else{
    $('#SearchForm').attr('action','plow/Excel_Import').submit();
  }
});
<? foreach ($this->tableful->Search as $SearchKey=> $SearchValue):?>
  <?if($SearchValue[0]==10):?>
